...
async function get() {
    ...
    let summaryText = `As of ${new Date(updatedAt).toLocaleString('en-NZ', { timeZone: 'Pacific/Auckland' })}, the Ministry of Health New Zealand reported in the past 24 hours;`;

    if (newActive > 0) summaryText += `\n - ${newActive === 1 ? '1 new case' : `${addCommas(newActive)} new cases`} of COVID-19, there are currently ${active} active cases in New Zealand.`;
    else if (active === '0') summaryText += '\n - No new cases of COVID-19, there are currently no active cases of COVID-19 in New Zealand.';
    else summaryText += `\n - No new cases of COVID-19, there is currently ${active} active cases in New Zealand.`;

    if (newRecovered > 0) summaryText += `\n - ${newRecovered === 1 ? '1 person has' : `${addCommas(newRecovered)} people have`} recovered from the virus, for a new total of ${recovered}.`;
    if (newDeceased > 0) summaryText += `\n - ${newDeceased === 1 ? '1 person has' : `${addCommas(newDeceased)} people have`} died from the virus, for a new total of ${deceased}.`;
